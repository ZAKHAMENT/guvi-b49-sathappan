...
useEffect(() => {
  ...
  fetchData();
    ...
    const updatePolarAreaChart = (label, data) => {
      // Extract the numeric part from the label (e.g., "Day 9" becomes "9")
      const dayNumber = label.match(/\d+/)[0];
    
      setPolarAreaChartData((prevChartData) => {
        return {
          labels: [...prevChartData.labels, label], // Keep the full date and time
          datasets: [
            {
    ...
    useEffect(() => {
      ...
      return () => {
        if (polarAreaChartInstance) {
          polarAreaChartInstance.destroy(); // Cleanup chart instance on component unmount
        }
      };
    }, [polarAreaChartData]);
    ...
    const handleScrollToPolarChart = () => {
      ...
      // Function to handle the scroll to the bar chart
