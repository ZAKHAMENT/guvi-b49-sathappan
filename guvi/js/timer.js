...
function reset_for_current_page(sublesson) {
    TimeMe.initialize({
        currentPageName: sublesson,
        idleTimeoutInSeconds: 60,
    });
    TimeMe.resetRecordedPageTime(sublesson);
}
$(document).ready(function() {
    ...
    var isTabActive = true
      , isActive = true;
    var data_json = {};
    starttimer(current_path[0]);
    var time, temp = [];
    function upload_local() {
        if (isTabActive === true && isActive === true) {
            time = givetime(current_path[0]);
            reset_for_current_page(current_path[0]);
            starttimer(current_path[0]);
            var data = {};
            data['time'] = time;
            data['pagename'] = current_path[0];
            data['type'] = "log_time_forpage";
            temp.push(data);
    ...
    function upload() {
        if (isTabActive === true) {
            data_json = localStorage.getItem("timer");
            var data = {};
            data_json = data_json ? JSON.parse(data_json) : '[]';
            data.timerData = data_json;
            authorize.invisibleAjax(data, 'timer', function(data) {
                reset_for_current_page(current_path[0]);
                starttimer(current_path[0]);
                temp = [];
                localStorage.setItem("timer", '[]');
    ...
    setInterval(upload_local, 180000);
