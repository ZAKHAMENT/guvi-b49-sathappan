...
useEffect(() => {
  const fetchData = async () => {
    try {
      const polarResponse = await axios.get('https://task-submission-d4k5.onrender.com/get-polar-chart-data', {
    ...
    try {
      const barResponse = await axios.get('https://task-submission-d4k5.onrender.com/get-bar-chart-data', {
        withCredentials: true,
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
  ...
  fetchData();
    ...
    const fetchBarChartData = async () => {
      try {
        const response = await axios.get('https://task-submission-d4k5.onrender.com/get-bar-chart-data', {
          withCredentials: true,
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
    ...
    useEffect(() => {
      ...
      useEffect(() => {
        const fetchSubmittedTasks = async () => {
          try {
            const response = await axios.get('https://task-submission-d4k5.onrender.com/get-submitted-tasks', {
              withCredentials: true,
              headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${localStorage.getItem('token')}`,
     ...
     const submitTask = async (newTask) => {
      try {
        ...
        const response = await axios.post(
          'https://task-submission-d4k5.onrender.com/submit-task',
          newTask,
          {
            withCredentials: true,
            headers: {
        ...
        // Handle the error more gracefully, you can set an error state here if needed
...
<li><Link to=''><i className="bi bi-chat-left-dots-fill"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-chat-left-dots-fill" viewBox="0 0 16 16">
  ...
  {/*____________________________________ Heading Bar _____________________________________________*/}
       ...
       <div className='top-bar'> <img className='gt-logo' src="https://i.pinimg.com/736x/47/21/71/4721713ecf391e39c753bff5c03a2c10.jpg" alt="" /><Link to="/login"><i className="bi bi-box-arrow-in-right"></i></Link><button className='log-btn' onClick={() => { handleLogout();  }}>Logout
