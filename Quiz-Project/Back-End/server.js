...
app.post('/api/user-temp-progress', async (req, res) => {
  const { userId, progress, tempTag } = req.body;

  if (!userId || !progress) {
    return res.status(400).send("Missing userId or progress data");
  }
  ...
  try {
    ...
    // Store the progress under the hashed key
    user.tempProgress.set(String(hashedCode), progress);
    await user.save();
...
app.post('/api/store-time', async (req, res) => {
  const { userId, time } = req.body;
  
  if (!userId || time === undefined) {
    return res.status(400).send("Missing userId or time data");
  }
  ...
  try {
    ...
    user.tempProgress.time = time;
    await user.save();

    // console.log(`Time saved for user ${userId}:`, time);
