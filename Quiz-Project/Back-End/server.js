...
// app.post('/api/frequent-time-saver', async (req, res) => {
//   const {userId, timer, isValidUrl} = req.body;
//     console.log(userId ,timer,isValidUrl);
    
//   if (!userId || !timer || !isValidUrl) {
//     return res.status(400).send("Missing userId or Time");
//   }
...
//     }
//     const existingProgress = user.tempProgress.find((p) => p.quizId === isValidUrl);
...
//         time: timer || existingProgress.time,
//       };
//     }
//     await user.save();
//     res.status(200).send("Frequent Time saved successfully!");
...
app.post('/api/count-attempts', async (req, res) => {
  ...
  if (!userId || !tag || !isValidUrl) {
    console.log('Req.body', userId, tag, isValidUrl);
...
app.post('/api/save-game-info', async (req, res) => {
  const { userId, userFinalResult, isValidUrl } = req.body;  
  
  if (!userId || !isValidUrl) {
    // console.log(userId, userFinalResult, isValidUrl);
    return res.status(400).send("Missing required fields: userId, userFinalResult, isValidUrl");
  }
  ...
  try {
    ...
    const index = user.tempProgress.findIndex(p => p.quizId === isValidUrl);
    user.tempProgress[index].result = userFinalResult;
    await user.save();
    res.status(200).send("Attempts saved successfully!");
  } catch (error) {
    console.error("Error saving Attempts:", error);
