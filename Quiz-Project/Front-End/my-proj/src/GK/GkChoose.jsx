...
const saveProgressToServer = async () => {
  ...
  const handleVisibilityChange = () => {
    if (document.visibilityState === "hidden") {
      // handleSubmitAns();
      // toast.error('Tab switched, you violated the game rules');
...
const clearForm = async () => {
  ...
  console.log(isValidUrl);
    ...
    let currNavigatedQuestionTrigger = true;
    let currNavigatedQuestion;
  const handleNavigateQuestions = (qNo) => {
    let call = new DoublyLinkedList();
    currNavigatedQuestion = call.findQuestion(qNo);
    console.log(currNavigatedQuestion);
    setCurrentQuestion(currNavigatedQuestion.data);
    currNavigatedQuestionTrigger = false;
  }

const handleNext = () => {
  const nextQuestion = questionsList.current.traverseForward();
  if (nextQuestion && currNavigatedQuestionTrigger) {
    setCurrentQuestion(nextQuestion);
    setSelectedOption(selectedOptions[nextQuestion.id] || null);
  } else {
    setCurrentQuestion(currNavigatedQuestion.next.data);
  }
};
...
<div className={styles.questionBtnContainer}>
  ...
  Prev</button>
    ...
    <button onClick={() => clearForm()} className={`${styles.btnPrev} ${styles.button}`}>clear form</button>
</div>
</div>
</div>
      )}
